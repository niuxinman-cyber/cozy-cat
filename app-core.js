...
function finishQuiz(){
  ...
  if(state.quizMode==='review'){if(needs.reviewFocusDay!==dayKey()){state.fish=clamp(state.fish-8);needs.reviewFocusDay=dayKey();resetFishClock();saveNeedsMeta();toast('完成复习：摸鱼值 -8 ✨')}}else{const token=dayKey()+':'+currentBatch();if(applyFocusOnce('quizFocus',token,10))toast('完成小测：摸鱼值 -10 ✨')}
