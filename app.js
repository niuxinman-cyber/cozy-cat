...
function renderStatus(){normalizeState();[['mood',state.mood],['food',state.full],['fish',state.fish]].forEach(([k,v])=>{const n=$('#'+k+'Num'),b=$('#'+(k==='food'?'food':k)+'Bar');if(n)n.textContent=Math.round(v)+'%';if(b)b.style.width=v+'%'});const pm=$('#profileMood');if(pm)pm.textContent=state.full>=75?'吃饱啦':state.mood>=80?'很开心':state.fish>=70?'在摸鱼':'在等你'}
...
function toggle(i){const learned=currentLearned();const p=learned.indexOf(i);if(p>=0)learned.splice(p,1);else{learned.push(i);const id=currentWords()[i]?.id;if(id&&!learning.seen.includes(id))learning.seen.push(id)}state.mood=clamp(state.mood+.8);renderWords();rewards();renderStatus();renderSummary();save()}
...
function renderSummary(){$('#sumQuiz').textContent=state.quizRewarded?'已领取':'未领取';$('#sumCat').textContent=state.full>=75?'吃饱啦':state.mood>=80?'很开心':state.fish>=70?'在摸鱼':'在等你'}
